#!/usr/bin/env python3
"""Export parity fixtures for PokemonState.hardened_initiative_bonus()."""
...
class PokemonStub:
    ...
    def hardened_initiative_bonus(self, battle=None) -> int:
        return self._pokemon_state_cls.hardened_initiative_bonus(self, battle)
...
def main() -> int:
    ...
    cases = [
        ...
        dict(name="hardened_many_injuries_same_bonus", injuries=9, current_round=4, expiry=None, feature=False, active=False, rank=0),
        dict(name="hardened_expired", injuries=3, current_round=4, expiry=3, feature=False, active=False, rank=0),
        dict(name="hardened_expires_this_round", injuries=3, current_round=4, expiry=4, feature=False, active=False, rank=0),
        dict(name="hardened_invalid_expiry_stays_active", injuries=3, current_round=4, expiry="not-a-round", feature=False, active=False, rank=0),
        ...
        dict(name="press_on_rank_eight_still_doubles", injuries=3, current_round=4, expiry=None, feature=True, active=True, rank=8),
    ...
    for case in cases:
        ...
        expected = pokemon.hardened_initiative_bonus(pokemon.battle)
        rows.append("\t".join([
            case["name"],
    ...
    output.write_text("\n".join(rows) + "\n", encoding="utf-8")
    print(f"wrote {len(cases)} Hardened initiative fixtures to {output}")
    return 0
