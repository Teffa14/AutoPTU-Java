...
tasks.test {
    ...
    if (oracleTurnFlow.isPresent) {
        systemProperty("autoptu.turnflow.oracle", oracleTurnFlow.get())
    }
    ...
    if (oracleActionSpace.isPresent) {
        systemProperty("autoptu.actionspace.oracle", oracleActionSpace.get())
    }
}
