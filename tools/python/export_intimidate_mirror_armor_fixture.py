#!/usr/bin/env python3
"""Freeze Intimidate interaction with Mirror Armor from the pinned Python oracle."""
...
def main() -> None:
    ...
    pokemon = {
        ...
        "target": PokemonState(spec=spec("Target", "Mirror Armor"), controller_id="b", position=(2, 3), active=True),
    }
    ...
    for event in battle.log:
        ...
        elif event.get("type") == "combat_stage":
            rows.append("\t".join([
                "STAGE",
                str(event.get("actor") or ""),
                str(event.get("target") or ""),
